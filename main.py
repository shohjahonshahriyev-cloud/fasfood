...
async def cmd_start(message: Message):
    user_name = message.from_user.full_name or message.from_user.first_name or message.from_user.username or "Foydalanuvchi"
    
    await message.answer(
        f"🍔 Xush kelibsiz, {user_name}!\n\n"
        "Mazali fast foodlarni tez buyurtma qiling",
...
async def product_selected(callback: CallbackQuery, state: FSMContext):
    ...
    if image_url and image_url.startswith("http"):
    ...
    elif image_url and len(str(image_url)) < 100:  # It's a file_id
        ...
        # Send without image (image_url is None or invalid)
        await callback.message.answer(
            text,
            reply_markup=keyboard,
            parse_mode=ParseMode.MARKDOWN
...
async def edit_product_name(callback: CallbackQuery, state: FSMContext):
    ...
    print(f"Debug: edit_name callback received - data: {callback.data}")
...
async def edit_product_description(callback: CallbackQuery, state: FSMContext):
    ...
    print(f"Debug: edit_description callback received - data: {callback.data}")
    await state.update_data(editing_field='description')
    await state.set_state(AdminStates.waiting_for_new_description)
...
async def edit_product_price(callback: CallbackQuery, state: FSMContext):
    ...
    print(f"Debug: edit_price callback received - data: {callback.data}")
...
async def edit_product_image(callback: CallbackQuery, state: FSMContext):
    ...
    print(f"Debug: edit_image callback received - data: {callback.data}")
    await state.update_data(editing_field='image')
    await state.set_state(AdminStates.waiting_for_new_image)
...
async def handle_new_name(message: Message, state: FSMContext):
    ...
    # Update product name
    success = db.update_product(product_id, name=message.text.strip())
    
    if success:
        await message.answer("✅ Mahsulot nomi muvaffaqiyatli yangilandi!")
    else:
        await message.answer("❌ Mahsulot nomini yangilashda xatolik!")
    
    await state.clear()
    await message.answer("👨‍💼 Admin menu:", reply_markup=get_admin_menu())

@dp.message(AdminStates.waiting_for_new_description)
async def handle_new_description(message: Message, state: FSMContext):
    ...
    success = db.update_product(product_id, description=message.text.strip() if message.text.strip() != 'skip' else None)
    
    if success:
        await message.answer("✅ Mahsulot tavsifi muvaffaqiyatli yangilandi!")
    else:
        await message.answer("❌ Mahsulot tavsifini yangilashda xatolik!")
    
    await state.clear()
    await message.answer("👨‍💼 Admin menu:", reply_markup=get_admin_menu())

@dp.message(AdminStates.waiting_for_new_price)
async def handle_new_price(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    ...
    if success:
        await message.answer("✅ Mahsulot narxi muvaffaqiyatli yangilandi!")
    else:
        await message.answer("❌ Mahsulot narxini yangilashda xatolik!")
    
    await state.clear()
    await message.answer("👨‍💼 Admin menu:", reply_markup=get_admin_menu())

@dp.message(AdminStates.waiting_for_new_image)
async def handle_new_image(message: Message, state: FSMContext):
    ...
    success = db.update_product(product_id, image_url=image_url)
    
    if success:
        await message.answer("✅ Mahsulot rasmi muvaffaqiyatli yangilandi!")
    else:
        await message.answer("❌ Mahsulot rasmini yangilashda xatolik!")
    
    await state.clear()
    await message.answer("👨‍💼 Admin menu:", reply_markup=get_admin_menu())

# Handle field updates for editing (legacy handler)
...
async def handle_edit_field(message: Message, state: FSMContext):
    ...
    if editing_field == 'name':
        ...
        if message.photo:
            photo = message.photo[-1]
            update_data['image_url'] = photo.file_id  # Store file_id
        elif message.text and message.text.lower() != 'skip':
            update_data['image_url'] = message.text
    elif editing_field == 'category':
        # Handle category change from setcat_ callback
        return  # This will be handled by callback
...
async def set_new_category(callback: CallbackQuery, state: FSMContext):
    ...
    if success:
        await callback.message.edit_text("✅ Mahsulot kategoriyasi muvaffaqiyatli o'zgartirildi!")
    else:
...
async def handle_unknown_callback(callback: CallbackQuery, state: FSMContext):
    ...
    known_patterns = [
        ...
        "new_category", "cancel_delete", "delcat_", "confirm_delcat_", "cancel_delcat", "setcat_"
    ]
